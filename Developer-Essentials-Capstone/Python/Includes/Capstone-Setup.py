...
userhome = f"dbfs:/user/{username}/dbacademy/dec"
...
def realityCheckBronze(writeToBronze):
  ...
  try:
    ...
    bronze_passed = bronze_tests.passed
...
def realityCheckStatic(loadStaticData):
  ...
  static_passed = static_tests.passed
...
def realityCheckSilver(bronzeToSilver):
  ...
  try:
    ...
    silver_passed = silver_tests.passed
...
def realityCheckFinal():
    ...
    suite = TestSuite()
    suite.testEquals(f"final.static-passed", "Reality Check Bronze passed", static_passed, True)
    suite.testEquals(f"final.bronze-passed", "Reality Check Static passed", bronze_passed, True)
    suite.testEquals(f"final.silver-passed", "Reality Check Silver passed", silver_passed, True)
    suite.testEquals(f"final.final-passed",  "Reality Check Gold passed",   gold_passed, True)
...
displayHTML("""
Declared the following function:
  <li><span style="color:green; font-weight:bold">realityCheckFinal</span></li>
""")
